...
public class MediaActivity extends BaseFragment implements SharedMediaLayout.SharedMediaPreloaderDelegate, FloatingDebugProvider, NotificationCenter.NotificationCenterDelegate {
    ...
    void lambda$onItemClick$0(ArrayList arrayList, AlertDialog alertDialog, int i) {
    ...
    public /* synthetic */ void lambda$createView$10(View view) {
        ...
        if (this.actionModeMessageObjects != null) {
            ...
            for (int i2 = 0; i2 < this.actionModeMessageObjects.size(); i2++) {
                TL_stories.StoryItem storyItem = this.actionModeMessageObjects.valueAt(i2).storyItem;
                if (storyItem != null) {
                ...
                if (closestTab >= 0) {
                    if (closestTab >= lastMediaCount.length || lastMediaCount[closestTab] >= 0) {
                        if (closestTab == 0) {
                            ...
                            int photosVideosTypeFilter = this.sharedMediaLayout.getPhotosVideosTypeFilter();
                            AnimatedTextView[] animatedTextViewArr = this.subtitleTextView;
                            if (photosVideosTypeFilter == 2) {
                                animatedTextViewArr[i].setText(LocaleController.formatPluralString("Videos", lastMediaCount[7], new Object[0]), z2);
                                return;
                            } else {
                                animatedTextViewArr[i].setText(LocaleController.formatPluralString("Media", lastMediaCount[0], new Object[0]), z2);
                                return;
                            }
                        }
                        if (closestTab == 1) {
    ...
    public List<FloatingDebugController.DebugItem> onGetDebugItems() {
        return Arrays.asList(new FloatingDebugController.DebugItem((ShapeDetector.isLearning(getContext()) ? "Disable" : "Enable").concat(" shape detector learning debug"), new Runnable() { // from class: org.telegram.ui.Components.MediaActivity$$ExternalSyntheticLambda14
            @Override // java.lang.Runnable
            public final void run() {
